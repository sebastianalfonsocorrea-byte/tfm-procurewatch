...
class BatchTests(unittest.TestCase):
    ...
    def test_monthly_batch_writes_freeze_manifest(self) -> None:
        with TemporaryDirectory() as temp:
            workspace = Path(temp)
            raw_dir = workspace / "data/raw"
            processed_dir = workspace / "data/processed"
            ...
            self.assertIsNotNone(result["freeze_manifest_path"])
            self.assertTrue(Path(result["freeze_manifest_path"]).exists())
            self.assertEqual(result["freeze_manifest"]["freeze_mode"], "final")
