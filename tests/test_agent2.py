...
class Agent2Tests(unittest.TestCase):
    def test_mvp_generates_traceable_flags_and_scores(self) -> None:
        ...
        with TemporaryDirectory() as temp:
            ...
            comparison = pd.read_parquet(report["outputs"]["model_comparison"])
            ...
            self.assertEqual(report["comparison_rows"], 5)
            self.assertEqual(
            ...
            self.assertIn("comparison_evaluation", report)
            self.assertEqual(report["comparison_evaluation"]["reference"], "rule_positive")
            self.assertEqual(report["comparison_evaluation"]["rows"], 5)
            self.assertIn("iforest", report["comparison_evaluation"])
            self.assertIn("pu", report["comparison_evaluation"])
            self.assertEqual(report["comparison_evaluation"]["iforest"]["support"], 5)
            self.assertEqual(report["comparison_evaluation"]["pu"]["support"], 5)
    ...
    def test_rf05_threshold_is_configurable(self) -> None:
        ...
        with TemporaryDirectory() as temp:
            ...
            self.assertEqual(report["comparison_rows"], 1)
